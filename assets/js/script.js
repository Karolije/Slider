    ...
    if (jsSliderThItem && jsSliderThumbs) {
        ...
        thumbsSrc.forEach(function (src) {
            ...
            if (cloneThItemSrc === currentImg) {
                console.log(cloneJsSliderThItem)
...
const onImageNext = function (event) {
    ...
    const nextEl = currentImage.parentElement.nextElementSibling;


    // 3. sprawdzić czy ten element istnieje - jeśli nie to [.nextElementSibling] zwróci [null]

    if (nextEl) {
        currentImage.classList.remove('nextElementSibling');
        nextEl.children[0].classList.add('nextElementSibling');


        // 4. przełączyć klasę [.js-slider__thumbs-image--current] do odpowiedniego elementu
        currentImage.classList.remove('js-slider__thumbs-image--current');
        nextEl.children[0].classList.add('js-slider__thumbs-image--current');


        // 5. podmienić atrybut o nazwie [src] dla [.js-slider__image]

        const newCurrentImg = nextEl.children[0].getAttribute('src');

        const jsSliderImage = document.querySelector('.js-slider__image');

        jsSliderImage.setAttribute('src', newCurrentImg);
    } else {
        const gallery = currentImage.parentElement.parentElement;
        console.log(gallery)

        currentImage.classList.remove('nextElementSibling');
        gallery.children[1].children[0].classList.add('nextElementSibling');
        currentImage.classList.remove('js-slider__thumbs-image--current');
        gallery.children[1].children[0].classList.add('js-slider__thumbs-image--current');

        const firstImg = gallery.children[1].querySelector('img').getAttribute('src');
        const jsSliderImage = document.querySelector('.js-slider__image');
...
const onImagePrev = function (event) {
    ...
    const previousEl = currentImage.parentElement.previousElementSibling;
    ...
    if (previousEl !== null && !previousEl.className.includes('js-slider__thumbs-item--prototype')) {
        ...
        currentImage.classList.remove('js-slider__thumbs-image--current');
        previousEl.children[0].classList.add('js-slider__thumbs-image--current');
        ...
        currentImage.classList.remove('js-slider__thumbs-image--current');
        lastChild.children[0].classList.add('js-slider__thumbs-image--current');
...
const onClose = function (event) {
    ...
    const thumbsChildren = thumbs.children;


    while (thumbsChildren.length > 1) {
        thumbs.removeChild(thumbs.lastElementChild);
    }
}
